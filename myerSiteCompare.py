...
class Url(object):
    ...
    def getUrlWithoutScheme(self):
        return u"%s" % urlunparse(('', self.netloc, self.path, self.params, urlencode(self.query), self.fragment))  
...
class siteCompare(object):
    ...
    def __init__(self, site1 = None, site2 = None, output = None, template = None, logLevel = logging.DEBUG, profile = None, profile1 = None, profile2 = None, security1 = None, security2 = None, ignoreLinks = None, ignoreParams = None, method = None):
        if output:
        ...
        if ignoreParams is not None:
            self._ignoreParams = ignoreParams        
            
        if method is not None:
            self._executeMethod = method
            
        if security1 is not None:
            self._security1 = security1
            
        if security2 is not None:
            self._security2 = security2
    ...
    def startSite1(self):
        ...
        #Windows Security for firefox
        self.driver.get(generateUrl(self._site1,self._security1))
    ...
    def startSite2(self):
        ...
        #Windows Security for firefox
        self.driver.get(generateUrl(self._site2,self._security2))
    ...
    def appendLinks(self, link):
        site1_path = Url(self._site1).path
        site2_path = Url(self._site2).path
        url = Url(link)
        url_path = url.path
            ...
            if path in self._ignoreLinks:
                ...
                if self._ignoreParams :
                    ...
                    for curpara in self._ignoreParams:
                        if ((link.find(curpara,0)) > -1) and ((link.find(curpara,0)) > -1) :
                            cur_ignore = 1
                    if cur_ignore == 1 :
                        logging.info('IgnoreParam link [%s].' % link)
                    else:
                        link = url.build()
    ...
    def readjson(self):
        ...
        #site1
        jsonPath = os.path.join(self._outputPath, self._site1Capture, self._site1Capture + '.json')
        fp = open(jsonPath)
        all_file = fp.read().replace(" ", "")
        all_data = all_file[1:-1].split(',')
        for i in range(0,len(all_data)):
            cur_data = all_data[i]
            cur_data = cur_data.split(':')
            self._site1Links[cur_data[0][1:-1]] = cur_data[1][1:-1]
            self._siteLinkCaptures[cur_data[1][1:-1]] = cur_data[0][1:-1]
        fp.close()
        
        #site2
        jsonPath = os.path.join(self._outputPath, self._site2Capture, self._site2Capture + '.json')
        fp = open(jsonPath)
        all_file = fp.read().replace(" ", "")
        all_data = all_file[1:-1].split(',')
        for i in range(0,len(all_data)):
            cur_data = all_data[i]
            cur_data = cur_data.split(':')
            self._site2Links[cur_data[0][1:-1]] = cur_data[1][1:-1]
            self._siteLinkCaptures[cur_data[1][1:-1]] = cur_data[0][1:-1]
        fp.close()
    ...
    def buildSiteLink(self, link, web = 'site1'):
        ...
        cur_url = None
        if 'site1' == web:
            cur_url = Url(self._site1)
        else:
            cur_url = Url(self._site2)
        url = urlunparse((cur_url.scheme,cur_url.netloc,link,'','',''))
    ...
    def compareResult(self, site1Link, site1Target, site2Link, site2Target, filename):
        logging.info("self._site1:[%s]" % site1Link)
        logging.info("self._site2:[%s]" % site2Link)
        ...
        site2Link = self.buildSiteLink(site2Link,'site2')
        site2Target = self.buildSiteLink(site2Target,'site2')
...
def main(args=None):
    ...
    parser.add_option('', '--security1', metavar="STRING", action='store', dest='security1',
        help='open site1 with windows security account and password')
    parser.add_option('', '--security2', metavar="STRING", action='store', dest='security2',
        help='open site2 with windows security account and password')        
    ...
    parser.add_option('', '--ignoreParam', metavar="STRING", action='store', dest='ignore2',
        help='ignore params, separated by commas')
    parser.add_option('-m', '--method', metavar="STRING", action='store', dest='method',
        help='just for result')
    ...
    _output = 'result' + time.strftime('%Y%m%d_%H%M',time.localtime(time.time()))
    if options.output:
    ...
    if options.security1:
        _security1 = options.security1
        
    _security2 = None
    if options.security2:
        _security2 = options.security2
    ...
    _ignoreParams = []
    if options.ignore2:
        _ignoreParams.extend(options.ignore2.split(','))
    ...
    # START
    sites = siteCompare(site1 = _site1, site2 = _site2, output = _output, template = _template, logLevel = _level, profile = _profile, profile1 = _profile1, profile2 = _profile2, security1 = _security1, security2 = _security2, ignoreLinks = _ignoreLinks, ignoreParams = _ignoreParams, method = _method)
    sites.run()
